    ...
    try {
       ...
       const {data}= await axios.post('https://clipdrop-api.co/text-to-image/v1',formData,{
        ...
        res.json({success: true, content:secure_url});
            ...
            console.error("ClipDrop error data:", error.response?.data); // This will show exact error message
            res.json({ success: false, message: error.message });
        }
...
export const removeImageObject = async (req, res) => {
    try {
        ...
        console.log("REQ PLAN:", plan);
        console.log("REQ FILE:", image);
        console.log("REQ BODY OBJECT:", object);

        if (plan !== 'premium') {
            return res.status(403).json({ success: false, message: 'This feature is only for premium users.' });
        }
        ...
        const imageUrl = cloudinary.url(uploaded.public_id, {
            transformation: [
                {
                    effect: `gen_remove:prompt:${object}`, // ✅ Fixed
                }
            ],
            resource_type: 'image',
        });
...
export const resumeReview = async (req, res) => {
    try {
        ...
        const content = response.choices[0].message.content;
         const promptText = 'Review the uploaded resume';

    await sql`
      INSERT INTO creations (user_id, prompt, content, type)
      VALUES (${userId}, ${promptText}, ${content}, 'resume-review')
    `;
