...
const WriteArticle = () => {
  ...
  const onSubmitHandler = async(e)=>{
    ...
    try {
      setLoading(true)
      const prompt = `Write an article about ${input} with a length of ${selectedLength.text} words.`
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg bg-white p-4 rounded-lg border border-gray-200 '>
        ...
        <input onChange={(e)=> setInput(e.target.value)} value={input} type='text' className='w-full p-2 px-3 mt-2 border border-gray-300 rounded-md outline-none test-sm 'placeholder='The future of artificial intelligence is ...' required />
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          ...
          <button disabled={loading}  className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#226BFF] to-[#65ADFF] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'>
            {loading ?  <span className='animate-spin w-4 h-4 border-2 border-t-transparent border-white rounded-full'></span> :
            <span className='w-5 h-5'><Sparkles className='w-5' /></span>}
            <Edit className='w-5'/>
             Generate Article
            </button>
      </form>
       ...
       <div className='w-full max-w-lg bg-white p-4 rounded-lg border border-gray-200 flex flex-col min-h-96 max-h-[600px]'>
          ...
          {!content ? (
            ...
            <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
              ...
              <p>Enter a topic and select the length of the article to generate content.</p>
            </div>
            </div>
          ) : (
