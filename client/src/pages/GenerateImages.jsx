...
const GenerateImages = () => {
  const imageStyle =['Realistic','Ghibli style', 'Anime style', 'Cartoon style', 'Cyberpunk', 'Fantasy style', 'Sci-Fi', 'Realistic style', '3D style', 'Portrait style']
      ...
      const onSubmitHandler = async(e)=>{
        ...
        try {
          ...
          const prompt = `Generate an image with the description: ${input} in the style of ${selectedStyle}.`
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg bg-white p-4 rounded-lg border border-gray-200 '>
        ...
        <textarea onChange={(e)=> setInput(e.target.value)} value={input} rows={4}  className='w-full p-2 px-3 mt-2 border border-gray-300 rounded-md outline-none test-sm 'placeholder='Describe what you want to see in the image...' required />
       ...
       <div className='w-full max-w-lg bg-white p-4 rounded-lg border border-gray-200 flex flex-col min-h-96 '>
          ...
          </div>
          {
            !content ? (
              ...
              <div>
                <img src={content} alt='Generated image' className='w-full h-auto object-cover rounded-lg mt-3' />
              </div>
            )
          }
