  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg bg-white p-4 rounded-lg border border-gray-200 '>
        ...
        <input onChange={(e)=> setInput(e.target.files[0])} type='file' accept='image/*' className='w-full p-2 px-3 mt-2 border border-gray-300 rounded-md outline-none test-sm text-gray-600'  required />
          
          <p className='mt-6 text-sm font-medium'>Describe Object name to remove</p>
          <textarea onChange={(e)=> setObject(e.target.value)} value={object} rows={4}  className='w-full p-2 px-3 mt-2 border border-gray-300 rounded-md outline-none test-sm 'placeholder='e.g., watch or spoon , only single object name' required />
       ...
       <div className='w-full max-w-lg bg-white p-4 rounded-lg border border-gray-200 flex flex-col min-h-96 '>
          ...
          </div>
          {
            ...
            <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
              ...
              <img src={content} alt='image' className='mt-3 h-full object-contain' />
            )
          }
